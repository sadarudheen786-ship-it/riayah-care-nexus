...
export function ConnectWhatsAppBusiness() {
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      setDetails(lines);
      setMessage(result.exchange.message);
    } catch (error) {
    ...
    <div className="rounded-xl border border-border bg-muted/30 p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-sm font-semibold text-foreground">Connect WhatsApp Business</div>
          ...
          <Button size="sm" className="mt-3 gap-1.5" onClick={start} disabled={busy}>
            {busy ? (
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
            ) : (
              <MessageCircle className="h-3.5 w-3.5" />
            )}
            {busy ? "Waiting for Meta…" : "Connect WhatsApp Business"}
          </Button>
